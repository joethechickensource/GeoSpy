    ...
    var removeFailureAlert = function (googleAPIcomponentURL) {
      sendRequestThroughCROSproxy(googleAPIcomponentURL, (responseText) => {
        ...
        script.innerHTML = responseText
          .replace(new RegExp(/;if.*Failure.*?\}/), ";")
          .replace(new RegExp(/(\|\|\(\(\)=>\{\}\);.*\?.*?\()/), "$1true||")
          .replace(
            /\{.*\/maps\/api\/js\/QuotaService.RecordEvent.*?\}\)\}/,
            "{}"
          );
...
sendRequestThroughCROSproxy(
  "https://maps.googleapis.com/maps/api/js?key=AIzaSyCFzXFZtPFUn-UgxPe3ZFY_5cJwcroHwAE&callback=initMap" +
    args,
  (googleAPIjs) => {
